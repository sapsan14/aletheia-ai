...
- **Подпись** привязывает дайджест к нашему приватному ключу; TSA затем ставит метку времени на **байты подписи** (см. [Временные метки](TIMESTAMPING.ru.md)).

Мы удостоверяем *что* было сказано; TSA удостоверяет *когда* это было подписано. См. [Модель доверия](TRUST_MODEL.ru.md) и [диаграммы](../diagrams/architecture.md).
...
- токеном временной метки (непрозрачные байты — см. [Временные метки](TIMESTAMPING.ru.md)),
...
**Архитектура** сделана так, чтобы тот же пайплайн можно было в будущем подключить к квалифицированным поставщикам доверенных услуг и к подписанию, совместимому с eIDAS (см. [Модель доверия — eIDAS](TRUST_MODEL.ru.md#eidas-неквалифицированное--квалифицированное)).

---

## Связанные документы

- [Временные метки](TIMESTAMPING.ru.md) — что метим по времени (байты подписи), TSA, хранение.
- [Модель доверия](TRUST_MODEL.ru.md) — цепочка доверия, кто что удостоверяет, соответствие eIDAS.
