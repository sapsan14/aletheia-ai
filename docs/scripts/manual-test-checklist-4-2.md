...
Checklist for **§4.2** of [Phase 4→5 transition plan](../internal/en/phase4-5-transition.md). Run with backend + frontend up; tick when done.

**Prerequisites:** Backend (`mvn spring-boot:run`), frontend (`npm run dev`), signing key + TSA in backend `.env`. See [Demo script](demo-script.md#prerequisites).
