...
- [Этап 1 — Настройка проекта и каркас](#этап-1--настройка-проекта-и-каркас)
- [Этап 2 — Крипто-слой](#этап-2--крипто-слой)
- [Этап 3 — Интеграция LLM](#этап-3--интеграция-llm)
- [Этап 4 — Аудит и персистентность](#этап-4--аудит-и-персистентность)
...
**Инструкция для кодинга (LLM-readable):** см. [docs/internal/en/plan.md](../en/PLAN.md#task-235--crypto-demo-endpoint-bridge-to-api).
...
| **Статус** | ✅ Done |
| **Оценка** | 4–5 ч |
| **Описание** | Запрос RFC 3161 временной метки у TSA (локальный сервер или заглушка). Вход: **байты подписи** (не текст ответа ИИ). Выход: TSA token, хранить как непрозрачные байты. |

**Дизайн (задокументировать в коде и README):**
- **Что метим по времени:** TSA ставит метку на **байты подписи**, а не на исходный ответ ИИ. Цепочка: `текст ИИ → hash → sign(hash) → timestamp(подпись)`. Мы удостоверяем содержание; TSA — момент времени.
- **Интерфейс:** TimestampService остаётся нейтральным: например `timestamp(byte[] dataToTimestamp)` — без AI-типов. Вызывающий передаёт `signatureBytes`; сервис — универсальный PKI-модуль.
- **Два режима:** (1) **REAL_TSA** — RFC 3161 HTTP к настраиваемому URL. (2) **MOCK_TSA** — детерминированная заглушка для юнит-тестов и CI без поднятия TSA (enterprise-подход к тестам).
- **Парсинг (опционально):** разобрать возвращённый TimeStampToken, извлечь genTime, залогировать — даёт «живое» время в логах.
- **Хранение и скоуп:** TSA token хранить как **непрозрачные байты** (byte[] или Base64). **Проверка токена вне скоупа** этой задачи — не реализовывать верификацию PKI.

**Инструкция для кодинга (LLM-readable):**
- Создать TimestampService с интерфейсом например `byte[] timestamp(byte[] dataToTimestamp)` — вход: данные для метки (в пайплайне: **байты подписи**). Выход: timestamp token byte[] или Base64; хранить как непрозрачные байты. BouncyCastle TSP: запрос по digest, HTTP POST к TSA, разбор ответа, извлечение token. Поддержать REAL_TSA (URL из конфига) и MOCK_TSA (детерминированная заглушка для тестов). Ошибки соединения и невалидный ответ: Optional или исключение; задокументировать. Опционально: парсить TimeStampToken, извлечь genTime, залогировать. Юнит-тест: token не пустой и парсится; с MOCK_TSA реальный сервер не нужен. Верификация TSA токена вне скоупа. Сам TSA-сервер не реализовывать (кроме минимальной заглушки при необходимости).
...
**Когда реализовывать:** При наличии 3+ REST endpoints или после Этапа 5 (POST /api/ai/ask, GET /api/ai/verify/:id). Подробности и LLM-readable prompt: см. [docs/internal/en/plan.md](../en/PLAN.md#task-73--swagger--openapi-implement-when-needed).
...
**Инструкция для кодинга (LLM-readable):** см. [docs/internal/en/plan.md](../en/PLAN.md#task-81--dockerfiles-for-backend-and-frontend).
...
**Инструкция для кодинга (LLM-readable):** см. [docs/internal/en/plan.md](../en/PLAN.md#task-82--extend-docker-compose-for-app-services).
...
**Инструкция для кодинга (LLM-readable):** см. [docs/internal/en/plan.md](../en/PLAN.md#task-83--ansible-playbook-for-vm-setup-and-deploy).
...
**Инструкция для кодинга (LLM-readable):** см. [docs/internal/en/plan.md](../en/PLAN.md#task-84--github-actions-workflow-for-deploy).
...
**Инструкция для кодинга (LLM-readable):** см. [docs/internal/en/plan.md](../en/PLAN.md#task-85--readme-and-deployment-docs).
