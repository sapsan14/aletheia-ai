...
|----------|--------------|----------|
| **Backend Aletheia** | *Содержание* — «этот дайджест соответствует этому каноническому ответу» | Цифровая подпись над хешем SHA-256 (RSA PKCS#1). См. [Подпись](SIGNING.ru.md). |
| **TSA (Time-Stamp Authority)** | *Время* — «эта подпись существовала в этот момент» | RFC 3161 метка времени над **байтами подписи** (не над сырым текстом). См. [Временные метки](TIMESTAMPING.ru.md). |
...
**Итог:** Заменить «наш ключ» на квалифицированный ключ подписи и «нашу TSA» на квалифицированную TSA; пайплайн (канонизация → хеш → подпись → метка времени → сохранение) остаётся тем же. См. [Подпись](SIGNING.ru.md) и [Временные метки](TIMESTAMPING.ru.md) для текущего скоупа и будущих работ.
...
- [Подпись](SIGNING.ru.md) — что подписываем, ключ, интерфейс, хранение.
- [Временные метки](TIMESTAMPING.ru.md) — что метим по времени, TSA, хранение, скоуп.
