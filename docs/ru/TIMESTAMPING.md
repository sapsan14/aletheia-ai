...
Документ описывает интеграцию временных меток в backend Aletheia: варианты TSA, переключение mock/real, обработка ошибок, тестирование, хранение и скоуп. Понятное объяснение, зачем tsaToken (а не просто поле времени): [Crypto reference (EN)](../en/CRYPTO_REFERENCE.md#why-tsatoken-not-a-simple-timestamp).
...
- [Видение и дорожная карта](VISION_AND_ROADMAP.md) — следующие шаги: доверенное время, multi-TSA, анкорирование.
